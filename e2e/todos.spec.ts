...
test.describe('Todo Example', () => {
  ...
  test('add, toggle, and delete todos', async ({ page }) => {
    ...
    const todoSection = page.locator('p:has-text("todos1.mirror.json")').locator('..');
    ...
    // Verify in file
    let fileContent = JSON.parse(await fs.readFile(TODOS_FILE, 'utf-8')) as TodoState;
    ...
    fileContent = JSON.parse(await fs.readFile(TODOS_FILE, 'utf-8')) as TodoState;
  ...
  test('file changes update UI', async ({ page }) => {
    ...
    await expect(todoSection.getByText('External todo 1')).toBeVisible({ timeout: 2000 });
    await expect(todoSection.getByText('External todo 2')).toBeVisible({ timeout: 2000 });

    // Verify the completed one has strikethrough
    const completedTodo = todoSection.locator('span:has-text("External todo 2")');
    ...
    await expect(todoSection.getByText('External todo 1')).not.toBeVisible({ timeout: 2000 });
    await expect(todoSection.getByText('External todo 2')).not.toBeVisible({ timeout: 2000 });
  });
});
