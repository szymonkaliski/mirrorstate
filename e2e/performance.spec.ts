...
test.describe("Performance", () => {
  ...
  test("counter renders minimum required times on initial load", async ({
    page,
  }) => {
    ...
    const counterLogs = consoleLogs.filter((log) =>
      log.includes("counter state"),
    );
  ...
  test("counter renders once per file update", async ({ page }) => {
    ...
    const updateLogs = consoleLogs.filter((log) =>
      log.includes("counter state 42"),
    );
    expect(updateLogs.length).toBeGreaterThanOrEqual(1);
    expect(updateLogs.length).toBeLessThanOrEqual(2);
  });

  test("counter does not re-render excessively during rapid updates", async ({
    page,
  }) => {
    const consoleLogs: string[] = [];
