...
export function mirrorStatePlugin(options: MirrorStatePluginOptions = {}): Plugin {
  const opts = {
    path: '/mirrorstate',
  ...
  return {
    ...
    configureServer(server) {
      ...
      logger.info(`MirrorState WebSocket listening on ws://localhost:${server.config.server.port || 5173}${wsPath}`);
      ...
      wss.on('connection', (ws) => {
        ...
        const pattern = Array.isArray(opts.filePattern) ? opts.filePattern : [opts.filePattern];
        ...
        ws.on('message', (message) => {
          try {
            ...
            if (lastHash === messageHash) {
              logger.debug(`Skipping duplicate message from ${clientId} for ${name}`);
              return;
            }
            ...
            logger.info(`Updated ${name} with state (from ${clientId}):`, state);
          } catch (error) {
    ...
    resolveId(id) {
      if (id === 'virtual:mirrorstate/config' || id === 'virtual:mirrorstate/initial-states') {
        return id;
      }
    },
